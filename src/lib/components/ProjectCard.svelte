<script lang="ts">
	...
	let { stat }: { stat: ClubStat } = $props();
</script>

<div class="flex w-full flex-col rounded-lg border border-[#ec3750] bg-[#1f1f27] p-4">
	<div class="flex gap-1 text-xl">
		<div class="font-bold">YSWS:</div>
		<div class="text-[#ec3750]">{stat.ysws}</div>
	</div>
	<div class="flex gap-1 text-xl">
		<div class="font-bold">Creator:</div>
		:
		<div class="text-[#ec3750]">{stat.email}</div>
	</div>
	<div class="flex gap-1 text-xl">
		<div class="font-bold">Date:</div>
		<div class="text-[#ec3750]">{stat.createdTime.toLocaleString()}</div>
	</div>
	<a
		href={stat.code_url}
		target="_blank"
		rel="noopener noreferrer"
		class="w0full flex flex-row items-center text-xl font-bold text-[#ec3750] hover:bg-[#ec3750] hover:text-[#fff6eb]"
		>Visit Code <svg
			fill-rule="evenodd"
			clip-rule="evenodd"
